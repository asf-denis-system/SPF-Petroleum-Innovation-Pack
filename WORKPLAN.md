...
**Порция по запросу пользователя 2026-09-08:** подготовлена [карточка первой практической задачи](docs/first-use-next-work-card.md): пользователь, ситуация, вход, выбор следующей работы и форма результата. Синтез выполнен; проверка на реальном случае и сопоставление с общими DPF не выполнены. Это ограниченная подготовительная работа, решение PFAD-001 не принято. Следующая порция — заполнить карточку на одном случае пользователя.
