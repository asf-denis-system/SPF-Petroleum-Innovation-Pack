# Petroleum Innovation DPF — Manifest

**Навигационная цепочка входа:** [Manifest](00-pack-manifest.md) → [Ontology](ontology.md) → [Full Pack Map](07-map/01-upstream-full-pack-map.md)

**Область:** Инженерная работа с инновациями в upstream-нефтедобыче

**Версия:** 0.2 (DPF scope)
**Статус:** Активная разработка
**Авторы:** Д. Асфандияров
**Основано на:** FPF и общем нефтяном DPF `SPF-Petroleum`

---

## О DPF

Это предметный фреймворк принципов для **создания, испытания, внедрения и масштабирования инноваций в upstream-нефтедобыче**. Он пригоден для:

- Формулирования проблем и возможностей, связанных с состояниями и переходами нефти
- Сравнения альтернативных технологий и методов
- Проектирования пилотов и критериев проверки эффекта
- Подготовки пакета свидетельств для gate-решений
- Квалификации условий применимости и безопасного масштабирования
- Ретроспективы инновационных проектов и программ

**Ключевая идея:** технология не равна эффекту. Инновацией считается квалифицированное изменение метода, связанное с проблемой, состоянием целевого объекта, свидетельствами пилота, границами применимости и решением о дальнейшем использовании.

---

## 🎯 Целевой срез (TargetSlice)

| Параметр | Значение |
|----------|---------|
| **Entity of Concern** | Нефтяная инновация как квалифицированное изменение метода или технологии |
| **Среда эффекта** | Переходы и состояния нефти; пласт, скважины и инфраструктура задают условия и ограничения |
| **Рабочий диапазон** | От регистрации проблемы/возможности до пилота, упаковки метода и тиражирования |
| **Ключевые роли** | Владелец проблемы, инженер инноваций, предметный эксперт, проектировщик пилота, ревьюер пакета свидетельств, владелец внедрения |
| **Ограничения** | Не покрывает всю нефтяную инженерию и не задаёт регламент конкретной компании |

---

## Metadata

```yaml
dpf_id: petroleum-innovation
dpf_name: Petroleum Innovation Domain Principle Framework
legacy_pack_id: upstream-petroleum
version: 0.2.0
fpf_basis: E.4.DPF
domain_basis: SPF-Petroleum
status: draft
maintainers:
  - name: Д. Асфандияров
    contact: asf@example.ru
created: 2026-02-19
last_updated: 2026-06-28
```

---

## Scope (Границы DPF)

### What This DPF Covers (Что покрывает этот DPF)

Этот DPF описывает предметные принципы нефтяных инноваций, которые помогают:

- связать инновационную работу с конкретной проблемой и состоянием объекта;
- не подменять эффект новизной технологии;
- удерживать альтернативы до обоснованного выбора;
- проектировать пилот как получение решающего пакета свидетельств;
- отделять переносимый метод от локальных условий применения;
- масштабировать только в квалифицированной области применимости.

DPF отвечает на вопросы:

1. **Какую проблему и изменение состояния должна поддержать инновация?**
2. **Какие альтернативные решения следует сравнить до выбора технологии?**
3. **Какие условия, baseline и свидетельства нужны для пилота?**
4. **Когда результат допускает тиражирование, а когда требует остановки или нового цикла?**

Физические и системные принципы разработки месторождения используются как ограничения и source basis, но не образуют самостоятельную цель этого DPF.

### What This DPF Does NOT Cover

❌ **Геология** — петрография, седиментология, структурное моделирование (это source-of-truth для входных условий)

❌ **Геофизика** — обработка сейсмики, интерпретация (заложена как компонента, но не развёрнута)

❌ **Экономика** — NPV, санкции, цены на нефть (это downstream evaluation)

❌ **Организационные структуры** — как организовать департамент разработки

❌ **Детальные методики расчётов** — триангуляция скважин, МГЭ, расчёты давления (это инженерные поддисциплины)

❌ **Нефтяная отрасль в целом** — геологоразведка, бурение, разработка, добыча, подготовка и экономика как полные предметные области

❌ **Локальные регламенты** — конкретные сроки, формы отчётности, оргструктура и маршруты согласования относятся к LPF организации

---

## Dependencies

### FPF and Petroleum DPF Grounding

**Тип отношения:** этот DPF специализирует общий нефтяной DPF и использует FPF как фундаментальную основу.

| Позиция | Фреймворк | Роль |
|---------|-----------|------|
| Foundation | FPF (First Principles Framework) | Задаёт базовые различения и архитектуру DPF/LPF |
| Broad domain | SPF-Petroleum | Общий DPF нефтяной отрасли и нефтепромысловый контекст |
| Narrow domain | petroleum-innovation (этот DPF) | Специализирует знания для нефтяных инноваций |
| Local use | LPF конкретной организации | Добавляет местные роли, инструменты, сроки и регламенты |

**Правило приоритета:** при конфликте фундаментальных различений FPF имеет приоритет. Общие нефтяные ограничения наследуются из `SPF-Petroleum`, а этот DPF уточняет только инновационный bounded context.

**Rationale:** см. [DRR.003](01-domain-contract/01C-design-rationale.md#drr003--spf-как-production-layer-поверх-fpf). Переход к более узкому DPF нефтяных инноваций зафиксирован в текущем manifest; отдельный DRR на замену старой формулировки пока ещё не оформлен.

### FPF Distinctions Used

| FPF Distinction | SPF Концепт | How Used in This Pack |
|-----------------|-------------|----------------------|
| **System** | U.Systems | Нефть как целевая система; пласт/скважины/ППД как управляющие системы |
| **State** | Состояние нефти | Измеримый факт о стадии нефти: локализована, добыта, подготовлена |
| **Dynamics** | Конвейер нефти | Сквозная цепочка состояний нефти от «запасы» до «НПЗ» |
| **Control/Intervention** | ГТМ / ППД / бурение | Вмешательства, переводящие нефть в следующее состояние |
| **Uncertainty** | U.Episteme / Неопределённость | ГДМ как гипотеза; L0–L3 confidence levels |
| **Feedback & Coupling** | Запаздывание реакции | Пласт реагирует с задержкой; взаимовлияния скважин |
| **Work product** | Артефакт-доказательство | Переход нефти засчитывается только при наличии артефакта (INV.001) |
| **Failure mode** | FM.* | Типовые ошибки при нарушении вторых принципов |

### Relationship to Other Frameworks

| Pack | Тип связи | Описание |
|------|-----------|----------|
| FPF — First Principles Framework | Upstream meta-framework | Базовая мета-онтология; приоритет при конфликтах |
| SPF-Petroleum | Broader domain DPF | Общий нефтяной контекст и ограничения |
| LPF организации | Local specialization | Местные процессы и правила применения этого DPF |

---

## 🔧 Ключевые работающие объекты области

### 1. **Физические вторые принципы** (Блок 1–3)
- Энергия конечна и её баланс определяет схему разработки
- Флюиды текут по градиентам давления и по пути наименьшего сопротивления
- Пласт гетерогенен; однородные модели — упрощение
- Неопределённость подземного пространства неустранима

### 2. **Системные вторые принципы** (Блок 2, 5)
- Любое вмешательство меняет траекторию всей системы (нет локального действия)
- Пласт реагирует с запаздыванием на воздействия
- Система стремится к новому равновесию после возмущения
- Информация теряется при передаче между субъектами

### 3. **Вторые принципы инноваций** (Блок 3)
- Технология ≠ эффект (результат зависит от состояния целевой системы)
- Лаборатория ≠ пласт (масштаб+контекст критичны)
- Инновация всегда увеличивает вариативность и неопределённость системы
- Универсальных решений не существует

### 4. **Вторые принципы организации** (Блок 5, 4)
- KPI меняют поведение системы и выбор разработчиков
- Калибровка маскирует неопределённость; history matching не гарантирует прогноз
- Модель — это гипотеза, а не отражение реальности

---

## 📚 Структура Pack'а

| Раздел | Назначение |
|--------|-----------|
| **01-domain-contract** | Ограниченный контекст, словарь, онтология; DRR (Design Rationale Records) |
| **02-domain-entities** | Вторые принципы по блокам (физические, системные, инновационные) |
| **03-methods** | Методы верификации принципов в проекте |
| **04-work-products** | План разработки, матрица решений, квалификационные документы |
| **05-failure-modes** | 18 типовых ошибок и их последствия |
| **06-sota** | Примеры из практики, уровни зрелости |
| **07-map** | Граф взаимосвязей между принципами |

---

## Content Summary

| Section | Item Count | Lifecycle | Status |
|---------|------------|-----------|--------|
| Design Rationale Records (DRR) | 5 | Shape | complete |
| Distinctions | 11 | Shape | complete |
| Roles | 6 | Shape | complete |
| Physical Principles | 5 | Shape | complete |
| System Principles | 3 | Shape | complete |
| Innovation Principles | 4 | Shape | complete |
| Org Principles | 2 | Shape | complete |
| Work Principles (OA.EW) | 2 | Shape | complete (OA.EW.001–002) |
| Register Bridge | 1 | Shape | complete (01D-register-bridge.md) |
| Methods | 13 | Shape | complete (index + retrospective layer + prioritization) |
| Work Products | 7 | Shape | complete (01..07 in `04-work-products/`) |
| Navigation Maps | 1 | Shape | complete (`07-map/01-upstream-full-pack-map.md`) |
| Failure Modes Catalog | 14 | Shape | complete (incl. FM.EW.001–002) |
| SoTA Annotations | 9 | Shape | complete |

---

## Extended Kinds (SPF.SPEC.001)

| Code | Name | Description | Folder |
|------|------|-------------|--------|
| PP | Physical Principle | Закон физики пластовой системы | `02-domain-entities/` |
| SP | System Principle | Системное свойство разработки | `02-domain-entities/` |
| IP | Innovation Principle | Принцип применения инноваций | `02-domain-entities/` |
| OP | Org Principle | Организационный принцип | `02-domain-entities/` |
| EW | Economic Work Principle | Принципы удельного Work и экономического моста | `02-domain-entities/` |

---

## Entity Index (SPF.SPEC.003)

> Auto-generated by `scripts/generate-map.py --manifest`. Do not edit manually.

| ID | Name | Kind | Summary | Status |
|----|------|------|---------|--------|
| UP.PP.001 | Энергия конечна | PP | Пластовая энергия исчерпаема; баланс энергии определяет схему разработки | draft |
| UP.PP.002 | Флюиды текут по градиентам | PP | Нефть, газ, вода идут в сторону меньшего давления по пути наименьшего сопротивления | draft |
| UP.PP.003 | Пласт гетерогенен | PP | Нет среднего пласта; есть зоны проницаемости, барьеры, трещины, линзы | draft |
| UP.PP.004 | Неопределённость неустранима | PP | Подземное пространство наблюдается косвенно; наблюдаемость редкая и через шум | draft |
| UP.PP.005 | Вторые принципы вытекают из физики | PP | Принципы формулируются как следствия физических законов, не как советы | draft |
| UP.SP.001 | Вмешательство меняет траекторию | SP | Любое действие (скважина, ППД, ГРП) изменяет распределение параметров системы | draft |
| UP.SP.002 | Запаздывание реакции | SP | Пласт реагирует с задержкой; эффект может проявиться через месяцы или годы | draft |
| UP.SP.003 | Система ищет равновесие | SP | После вмешательства пласт перестраивается; траектория зависит от начальных условий | draft |
| UP.IP.001 | Технология ≠ эффект | IP | Результат определяется состоянием целевой системы, а не только технической новизной | draft |
| UP.IP.002 | Масштаб критичен | IP | Лабораторная правда, модельная правда и полевая правда различаются | draft |
| UP.IP.003 | Инновация увеличивает вариативность | IP | Новое ГТМ расширяет пространство возможных состояний и неопределённость | draft |
| UP.IP.004 | Универсальных решений не существует | IP | Применимость технологии ограничена состоянием и контекстом системы | draft |
| UP.OP.001 | KPI меняют поведение | OP | Метрика определяет выбор разработчика и траекторию развития месторождения | draft |
| UP.OP.002 | Информация теряется при передаче | OP | Потеря контекста между ролями и уровнями решений искажает управление | draft |
| UP.EW.001 | Удельный Work перехода | EW | Затраты оцениваются по переходам конвейера в физических и экономических регистрах | draft |
| UP.EW.002 | Обводнённость как мультипликатор | EW | Рост WC нелинейно увеличивает удельные затраты на тонну нефти | draft |

---

## Change Log (Pack-Specific)

| Date | Change | Author |
|------|--------|--------|
| 2026-02-19 | Начальная создание pack structure; агрегация 21 принципа | Асфандияров |
| 2026-02-21 | Добавлена модель трёх конвейеров (WP.ThreePipelines); новые различения D.009, D.010; новая роль R.006 (Владелец стыка); расширен bounded context | — |
| 2026-02-21 | Переопределена целевая система: нефть (не пласт); пласт/скважины/инфра — управляющие системы; обновлены bounded context v0.2.0, D.001, manifest | — |
| 2026-02-22 | FPF compliance: добавлен DRR (01C, 4 записи DRR.001–004); обновлены lifecycle статусы (Explore/Shape/Evidence/Operate); добавлен FPF→SPF Bridge в manifest | Асфандияров |
| 2026-02-22 | Economic Work extension: Register Bridge (01D), Work-Ledger шаблон (WP.04), OA.EW.001–002, FM.EW.001 — разделение физического и экономического регистров | Асфандияров |
| 2026-03-08 | Интеграционная синхронизация W11: заполнены ontology/tools/map, выровнены content-summary и entity-index под фактический состав Pack | Асфандияров |
| 2026-03-10 | Синхронизация diagnostic-layer: добавлен DRR.005 про ГДИ как метод vs контур, обновлён tools index и bounded context | Асфандияров |
| 2026-03-13 | Добавлен retrospective-layer: методы `UP.M.011..012` и шаблоны `WP.RetroProject/WP.RetroProgram` на основе импортированных методик ретроспективы | Асфандияров |
| 2026-03-13 | Добавлен метод `UP.M.013` и шаблон `WP.PriorityPortfolio` для problem-first приоритизации инновационных проектов | Асфандияров |
